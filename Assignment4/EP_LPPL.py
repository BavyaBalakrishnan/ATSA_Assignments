...
# class for a set of parameters, representing an individual
class chromosomes:
...
class population:
    ...
    # constructor
    def __init__(self, ep_parameters, verbose=True):
        (limits, pool_max_size, eliminate_ratio, mutate_ratio, num_mutation_points) = ep_parameters
        self.verbose = verbose
...
def generate_ep_parameters(timeseries,
    ...
    ep_parameters = (limits, pool_max_size, eliminate_ratio, mutate_probability, num_mutation)
    return ep_parameters

# execute the evolution
def execute(generations, ep_parameters, initial_p=False, verbose=True):
    if initial_p:
      ...
      p = population(ep_parameters, verbose=verbose)
...
def multi_steps(ticker, start_date, end_date, max_term, min_term, prediction_term, generations, verbose):
    ...
    for learning_end_date in datetimes_all[max_term:(max_term+NumOfRuns)]:
      ...
      ep_parameters = generate_ep_parameters(TIMESERIES, min_term=min_term)
      # execute the ga process
      #p = execute(generations, ep_parameters, initial_p=p, verbose=verbose) #, p)
      p = execute(generations, ep_parameters, verbose=verbose) #, initial_p=False)
      # record results
